...
export default function AdminCreateUserButton() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setRoles(data || []);
        if (data?.[0]?.name) setForm((f) => ({ ...f, role: data[0].name }));
      } catch (e: any) {
  ...
  async function createUser() {
    if (!form.email || !form.password) {
      toast.error('Email và password bắt buộc');
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    <>
      <button
        ...
        disabled={busy}
        className="ml-3 rounded bg-white text-black text-sm px-3 py-1.5 disabled:opacity-60"
      >
        New user
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-black/80 p-5 space-y-4">
            <h3 className="text-base font-semibold text-white">Tạo người dùng</h3>
